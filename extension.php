...
return [
	...
	*/
	...
	{
		Route::group([
				'prefix'    => admin_uri().'/products',
				'namespace' => 'Jiro\Extension\Product\Controllers\Admin',
			], function()
			{
				Route::get('/' , ['as' => 'admin.jiro.product.all', 'uses' => 'ProductController@index']);
				Route::post('/', ['as' => 'admin.jiro.product.all', 'uses' => 'ProductController@executeAction']);

				Route::get('grid', ['as' => 'admin.jiro.product.grid', 'uses' => 'ProductController@grid']);

				Route::get('create' , ['as' => 'admin.jiro.product.create', 'uses' => 'ProductController@create']);
				Route::post('create', ['as' => 'admin.jiro.product.create', 'uses' => 'ProductController@store']);

				Route::get('{id}'   , ['as' => 'admin.jiro.product.edit'  , 'uses' => 'ProductController@edit']);
				Route::post('{id}'  , ['as' => 'admin.jiro.product.edit'  , 'uses' => 'ProductController@update']);

				Route::delete('{id}', ['as' => 'admin.jiro.product.delete', 'uses' => 'ProductController@delete']);
			});

		Route::group([
			'prefix'    => 'products',
			'namespace' => 'Jiro\Extension\Product\Controllers\Frontend',
		], function()
		{
	...
	*/
	...
	'permissions' => function(Permissions $permissions)
	{
		$permissions->group('product', function($g)
		{
			...
			{
				$p->label = trans('jiro/product::permissions.index');

				$p->controller('Jiro\Extension\Product\Controllers\Admin\ProductController', 'index, grid');
			});

			$g->permission('product.create', function($p)
			{
				$p->label = trans('jiro/product::permissions.create');

				$p->controller('Jiro\Extension\Product\Controllers\Admin\ProductController', 'create, store');
			});

			$g->permission('product.edit', function($p)
			{
				$p->label = trans('jiro/product::permissions.edit');

				$p->controller('Jiro\Extension\Product\Controllers\Admin\ProductController', 'edit, update');
			});

			$g->permission('product.delete', function($p)
			{
				$p->label = trans('jiro/product::permissions.delete');

				$p->controller('Jiro\Extension\Product\Controllers\Admin\ProductController', 'delete');
			});
		});
	},
	...
	*/
	...
	'menus' => [
		...
		'admin' => [
			[
				'slug' 	=> 'admin-jiro-product',
				'name' 	=> 'Products',
				'class' => 'fa fa-codepen',
				'uri' 	=> 'products',
				'regex' => '/:admin\/products/i',
			],
		],
		'main' => [
